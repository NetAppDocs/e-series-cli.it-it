...
Tutti i valori massimi degli ID slot sono 24. I valori degli ID slot iniziano da 0 o da 1, a seconda del modello del vassoio. I vassoi per unità compatibili con i controller E2800, E5700, EF600, EF300, EF50 e EF80 hanno numeri di ID slot che iniziano da 0. I vassoi per unità compatibili con i controller E2700 e E5600 hanno numeri di ID slot che iniziano da 1.
...
`resourceProvisioningCapable`
 a| 
Se questo parametro è impostato su `TRUE`, verranno utilizzate solo unità compatibili con DULBE per creare pool di dischi o gruppi di volumi. Se il parametro è impostato su `FALSE` o non specificato, non viene eseguito alcun controllo sulla compatibilità con DULBE. Se tutte le unità selezionate per il gruppo di volumi o il pool di dischi sono compatibili con DULBE, il flag a livello di storage array determina se il gruppo di volumi o il pool di dischi viene creato come risorsa con provisioning.
